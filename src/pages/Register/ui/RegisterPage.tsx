...
export default function RegisterPage() {
    ...
    return (
        <Container className="flex flex-col justify-center gap-y-8 items-center h-full">
            <h1 className="text-2xl text-gray-800 font-medium">Регистрация</h1>
            <div onSubmit={() => HandleLogin(loginData.login, loginData.password)}
                ...
                <Button onClick={() => HandleLogin(loginData.login, loginData.password)}
                        className="bg-indigo-600 text-white py-2 duration-500 rounded-md hover:bg-indigo-800 mt-4">Зарегестрироваться
                </Button>
            </div>
            <span>Уже есть аккаунт? <Link className="text-indigo-600" href="/auth/login">Войти</Link></span>
        </Container>
    )
}